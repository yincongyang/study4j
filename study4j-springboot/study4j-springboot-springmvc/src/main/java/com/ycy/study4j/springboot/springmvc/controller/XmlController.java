...
/**
 * 返回xml格式的controller Created by yincongyang on 17/11/13.
 */
@Controller
@RequestMapping("/xml")
public class XmlController {
   ...
   * @param user
   * @return
   * @ResponseBody 通过适当的HttpMessageConverter转换为指定格式后，写入到Response对象的body数据区。 使用时机：
   ...
   * @param user
   * @return
   * @RequestBody: 该注解常用来处理Content-Type不是application/x-www-form-urlencoded编码的内容，例如application/json,
   * application/xml等； 它是通过使用HandlerAdapter 配置的HttpMessageConverters来解析post data
   * body，然后绑定到相应的bean上的。
   */
   ...
   * @param house
   * @return
   * @RequestMapping path:路径 value=path method:指定请求方式，使用RequestMethod.* produces:指定响应头中的ContentType
   * consumes:指定请求头中的ContentType params:指定Request中的参数名称，用于将同一URL映射至不同方法.eg：params="name=k"
   */
