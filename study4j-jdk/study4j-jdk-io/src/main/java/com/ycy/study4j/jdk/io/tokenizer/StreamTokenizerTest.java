package com.ycy.study4j.jdk.io.tokenizer;
...
/**
 * StreamTokenizer默认按照空格进行文字的分割，可以根据其进行英文字符分割。统计字数，词频等 下面是统计文件中字符个数的示例：
 * <p>
 ...
 * 统计文章字符数的程序，不是简单的统计Token数就万事大吉，因为字符数不等于Token。按照Token的规定， 引号中的内容就算是10页也算一个Token。如果希望引号和引号中的内容都算作Token，应该调用下面的代码：
      ...
      while (st.nextToken() != StreamTokenizer.TT_EOF) {
        ...
        switch (st.ttype) {
          ...
          case StreamTokenizer.TT_NUMBER:
            //如果当前标记是一个数字，nval字段将包含该数字的值 nval默认是double类型，若是11则为11.0
            s = String.valueOf((int) st.nval);//将double转为int
            numberSum += s.length();
            break;
