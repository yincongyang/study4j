 ...
 * <p>
 * FileInputStream 是文件输入流，它继承于InputStream。通常，我们使用FileInputStream从某个文件中获得输入字节。 FileOutputStream
 * 是文件输出流，它继承于OutputStream。通常，我们使用FileOutputStream 将数据写入 File 或 FileDescriptor 的输出流。
 *
 * @author yincongyang
 */
public class FileStreamTest {
   ...
   * <p>
   * 加入，我们将 FileOutputStream fileOut2 = new FileOutputStream(file, true); 修改为 FileOutputStream
   * fileOut2 = new FileOutputStream(file, false); 然后再执行程序，“file.txt”的内容变成"0123456789"。 原因是： (01)
   * FileOutputStream fileOut2 = new FileOutputStream(file, true); 它是以“追加模式”将内容写入文件的。即写入的内容，追加到原始的内容之后。
