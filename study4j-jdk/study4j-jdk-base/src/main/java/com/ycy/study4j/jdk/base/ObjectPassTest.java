 ...
 * java 按值传递和按引用传递问题
 * <p>
 * 对象传递方式： - 通过“=”进行赋值操作 - 通过方法参数传递
 * <p>
 * 传递类型： - 按值传递：传递的是值本身，不会出现两个字面量实际指向同一个对象的问题。 - 按引用传递(句柄||指针)：传递是的对象的地址，会出现多个字面量指向同一个对象。 -
 * java中除了基本数据类型的传递外，其余均为对象，所以都是按引用传递。
 * <p>
 * 别名问题： - 按引用传递带来的问题，当a和b同时指向同一个对象时，若修改了b指向对象本身的值，则也会修改a的值。这种场景有可能带来意料之外的效果。 -
 * 解决方案：使用Object.clone()。//使用较繁琐，尽量不要使用 - 创建不可变类(即所有内部成员变量均为private且不提供方法对其进行修改)及其对应修改类(类似String，StringBuffer)。
 *
 * @author yincongyang
 */
